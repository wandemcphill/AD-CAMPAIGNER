...
export function CommandBar({ isGenerating, onGenerate, prompt }: CommandBarProps) {
  ...
  return (
    <form
      className="group relative mx-auto mt-6 w-full max-w-3xl rounded-md border border-white/12 bg-white/[0.07] p-2 shadow-[0_30px_120px_rgba(0,0,0,0.5)] backdrop-blur-2xl"
      onSubmit={submit}
    >
      <div className="absolute inset-x-8 -top-px h-px bg-[linear-gradient(90deg,transparent,var(--flip-amber),var(--flip-indigo),transparent)]" />
      ...
      <div className="grid gap-2 sm:grid-cols-[auto_1fr_auto] sm:items-center">
        ...
        />
        <button
          className="flex h-12 items-center justify-center gap-2 rounded-md bg-[var(--flip-amber)] px-5 text-sm font-bold text-[#050507] transition hover:bg-white disabled:cursor-wait disabled:bg-white/50"
