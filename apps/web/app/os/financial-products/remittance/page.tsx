    ...
    <motion.div animate={{ opacity: 1 }} className="mt-6" initial={{ opacity: 0 }}>
      ...
      <Panel className="p-5">
        ...
        {quote && <div className="mt-4 space-y-3 rounded-[var(--radius-md)] border border-[var(--ft-border)] p-3">
          ...
          <Button className="w-full justify-center" disabled={!recipientName.trim() || !recipientAccountNumber.trim() || !recipientBankCode.trim() || sending} onClick={() => void submitSendRemittance()}><Send className="size-4" />{sending ? "Sending..." : "Send transfer"}</Button>
        </div>}
      </Panel>
