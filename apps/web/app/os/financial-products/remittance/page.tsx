...
export default function RemittanceTabPage() {
  ...
  return (
    <CustomerJourney
      ...
      description="Start with the amount, see the rate and fee, add the recipient, then confirm. No guessing what happens next."
      icon={Send}
      steps={[{ label: "Amount", description: "Choose what you want to send." }, { label: "Quote", description: "Review rate and fee." }, { label: "Recipient", description: "Add destination details." }, { label: "Confirm", description: "Review and send." }]}
      currentStep={quote ? 2 : 0}
    >
      <ErrorNotice message={error} />
      <Panel className="p-5">
        <div className="grid grid-cols-2 gap-3">
          <div><label className="mb-1 block text-xs text-[var(--ft-text-muted)]">Send (NGN)</label><input className="h-12 w-full rounded-[var(--radius-lg)] border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] px-4 text-sm outline-none focus:border-[var(--ft-accent)]" min={100} onChange={(e) => setSourceNaira(Number(e.target.value))} type="number" value={sourceNaira} /></div>
          <div><label className="mb-1 block text-xs text-[var(--ft-text-muted)]">To</label><select className="h-12 w-full rounded-[var(--radius-lg)] border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] px-4 text-sm outline-none focus:border-[var(--ft-accent)]" onChange={(e) => setDestinationCurrency(e.target.value)} value={destinationCurrency}><option value="USD">USD</option><option value="GBP">GBP</option><option value="EUR">EUR</option></select></div>
        </div>
        <Button className="mt-3 w-full justify-center" disabled={sourceNaira < 100 || quoting} onClick={() => void submitGetQuote()} variant="secondary">{quoting ? "Getting quote..." : "Get quote"}</Button>
        {quote && <div className="mt-4 space-y-3 rounded-[var(--radius-md)] border border-[var(--ft-border)] p-3">
          <div className="text-sm">Recipient gets <span className="font-semibold">{quote.destinationCurrency} {(quote.destinationAmountMinor / 100).toLocaleString()}</span> · fee {formatNaira(quote.feeMinor)}</div>
          <div className="text-xs text-[var(--ft-text-muted)]">You&rsquo;ll be charged {formatNaira(quote.sourceAmountMinor)}{!quote.isLocked && " · rate is indicative until the transfer completes"}</div>
          <input className="h-11 w-full rounded-[var(--radius-lg)] border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] px-4 text-sm outline-none placeholder:text-[var(--ft-text-muted)] focus:border-[var(--ft-accent)]" onChange={(e) => setRecipientName(e.target.value)} placeholder="Recipient name" value={recipientName} />
          <input className="h-11 w-full rounded-[var(--radius-lg)] border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] px-4 text-sm outline-none placeholder:text-[var(--ft-text-muted)] focus:border-[var(--ft-accent)]" onChange={(e) => setRecipientAccountNumber(e.target.value)} placeholder="Account number" value={recipientAccountNumber} />
          <div className="grid grid-cols-2 gap-2"><input className="h-11 w-full rounded-[var(--radius-lg)] border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] px-4 text-sm outline-none placeholder:text-[var(--ft-text-muted)] focus:border-[var(--ft-accent)]" onChange={(e) => setRecipientBankCode(e.target.value)} placeholder="Bank code" value={recipientBankCode} /><input className="h-11 w-full rounded-[var(--radius-lg)] border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] px-4 text-sm outline-none placeholder:text-[var(--ft-text-muted)] focus:border-[var(--ft-accent)]" onChange={(e) => setRecipientCountry(e.target.value)} placeholder="Country (e.g. US)" value={recipientCountry} /></div>
