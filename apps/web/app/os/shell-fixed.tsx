  ...
  const renderSection = (section: NavSection, mobile = false) => {
    ...
    return (
      <div key={section.href}>
        <a
          className={cn(
            "flex items-center gap-2.5 rounded-[var(--radius-md)] px-3 text-sm font-medium",
            mobile ? "h-10" : "h-9 text-[13px]",
            active ? "bg-[var(--ft-accent)]/10 text-[var(--ft-accent)]" : "text-[var(--ft-text-secondary)] hover:bg-[var(--ft-bg-muted)]"
          )}
          href={section.href}
        ...
        {active && (section.children?.length ?? 0) > 0 && (
          <div className={cn("my-1 flex flex-col gap-0.5 border-l border-[var(--ft-border)] pl-2", mobile ? "ml-6" : "ml-[22px]")}>
            {(section.children ?? []).map((child) => (
              <a
                className={cn(
                  "flex items-center gap-2 rounded-[var(--radius-md)] px-3 text-[13px]",
                  mobile ? "h-9" : "h-8 text-[12.5px]",
                  isActive(child.href) ? "font-medium text-[var(--ft-accent)]" : "text-[var(--ft-text-secondary)] hover:text-[var(--ft-text-primary)]"
                )}
                href={child.href}
                key={child.href}
                onClick={mobile ? () => setSidebarOpen(false) : undefined}
  ...
  return (
    <div className="flex min-h-screen bg-[var(--ft-bg-base)] text-[var(--ft-text-primary)]">
      {sidebarOpen && (
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <aside className="absolute inset-y-0 left-0 w-[280px] overflow-y-auto border-r border-[var(--ft-border)] bg-[var(--ft-bg-surface)] p-3">
            <div className="mb-4 flex items-center justify-between px-2"><span className="text-sm font-bold">FlipTrybe</span><button onClick={() => setSidebarOpen(false)} type="button"><X className="size-5" /></button></div>
            {visiblePrimary.map((section) => renderSection(section, true))}
            <div className="mt-4 border-t border-[var(--ft-border)] pt-3">
              {visibleAccount.map((item) => <a className="flex h-10 items-center gap-3 rounded-[var(--radius-md)] px-3 text-sm text-[var(--ft-text-secondary)]" href={item.href} key={item.href} onClick={() => setSidebarOpen(false)}><item.icon className="size-4" />{item.label}</a>)}
              <button className="mt-2 flex h-10 w-full items-center gap-3 rounded-[var(--radius-md)] px-3 text-sm text-[var(--ft-text-secondary)]" disabled={signingOut} onClick={() => void handleSignOut()} type="button"><LogOut className="size-4" />{signingOut ? "Signing out..." : "Logout"}</button>
            </div>
          </aside>
        </div>
      )}
      ...
      <aside className="hidden w-[240px] shrink-0 flex-col border-r border-[var(--ft-border)] bg-[var(--ft-bg-surface)] lg:flex">
        <div className="flex h-14 items-center gap-3 border-b border-[var(--ft-border)] px-5"><img alt="FlipTrybe" className="size-8" src="/brand/icon-mark.svg" /><div><div className="text-sm font-bold">FlipTrybe</div><div className="font-mono text-[9px] uppercase tracking-[0.12em] text-[var(--ft-text-muted)]">Growth OS</div></div></div>
        <nav className="flex-1 overflow-y-auto px-3 py-3">{visiblePrimary.map((section) => renderSection(section))}<div className="mt-4 border-t border-[var(--ft-border)] pt-3">{visibleAccount.map((item) => <a className={cn("flex h-9 items-center gap-2.5 rounded-[var(--radius-md)] px-3 text-[13px] font-medium", isActive(item.href) ? "bg-[var(--ft-accent)]/10 text-[var(--ft-accent)]" : "text-[var(--ft-text-secondary)] hover:bg-[var(--ft-bg-muted)]")} href={item.href} key={item.href}><item.icon className="size-4" />{item.label}</a>)}</div></nav>
        <div className="border-t border-[var(--ft-border)] p-3"><div className="flex items-center gap-3 rounded-[var(--radius-md)] px-3 py-2"><div className="grid size-8 place-items-center rounded-full bg-[var(--ft-accent)]/10 text-xs font-bold text-[var(--ft-accent)]">{session.user.name?.[0] ?? "U"}</div><div className="min-w-0 flex-1"><div className="truncate text-sm font-medium">{session.user.name}</div><div className="truncate text-[11px] text-[var(--ft-text-muted)]">@{session.user.username}</div></div><ThemeToggle /></div><button className="mt-1 flex h-9 w-full items-center justify-center gap-2 rounded-[var(--radius-md)] border border-[var(--ft-border)] bg-[var(--ft-bg-muted)] px-3 text-xs font-medium text-[var(--ft-text-secondary)]" disabled={signingOut} onClick={() => void handleSignOut()} type="button"><LogOut className="size-3.5" />{signingOut ? "Signing out..." : "Logout"}</button></div>
      </aside>
