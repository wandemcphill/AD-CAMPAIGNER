      ...
      {STAGES.map((stage, index) => {
        ...
        return (
          <div className="min-w-0" key={stage.id}>
            <div className={cn("h-1 rounded-full transition-colors", complete || active ? "bg-[var(--ft-accent)]" : "bg-[var(--ft-border)]")} />
            <div className={cn("mt-2 flex items-center gap-1.5 text-[10px] font-semibold", active ? "text-[var(--ft-text-primary)]" : "text-[var(--ft-text-muted)]")}>
              <span className={cn("grid size-4 shrink-0 place-items-center rounded-full text-[8px]", complete ? "bg-[var(--ft-green)]/15 text-[var(--ft-green)]" : active ? "border border-[var(--ft-accent)] text-[var(--ft-accent)]" : "border border-[var(--ft-border)] text-[var(--ft-text-muted)]")}>
                {complete ? <Check className="size-2.5" /> : index + 1}
              </span>
              <span className="truncate">{stage.label}</span>
            </div>
            {!compact ? <div className="mt-0.5 truncate pl-5 text-[9px] leading-3 text-[var(--ft-text-muted)]">{stage.description}</div> : null}
          </div>
        );
      })}
    </div>
