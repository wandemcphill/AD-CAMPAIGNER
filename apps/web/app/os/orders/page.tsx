...
import { loadAllOrders, orderSourceLabels, type UnifiedOrder, type UnifiedOrderSource } from "./api";
...
export default function OrdersPage() {
  ...
  async function refresh() {
    setLoading(true);
    try { const result = await loadAllOrders(); setOrders(result.orders); setUnavailable(result.unavailable); }
  ...
  const grouped = useMemo(() => {
    ...
    visible.forEach((order) => { const month = monthOf(order.createdAt); groups.set(month, [...(groups.get(month) ?? []), order]); });
    return Array.from(groups.entries()).map(([month, items]) => ({ items, month }));
  }, [visible]);

  return <>
    <header className="flex flex-wrap items-start justify-between gap-4">
      <div><div className="flex flex-wrap items-center gap-2"><Badge tone="info">Your activity</Badge><Badge tone="neutral">All verticals</Badge></div><h1 className="mt-2 text-2xl font-semibold text-[var(--ft-text-primary)]">Orders & activity</h1><p className="mt-1 max-w-2xl text-sm leading-6 text-[var(--ft-text-secondary)]">One place to follow what you bought, what is processing and what has completed across FlipTrybe.</p></div>
    ...
    <section className="mt-5 rounded-[26px] border border-[var(--ft-border)] bg-[var(--ft-bg-raised)] p-5 shadow-[var(--shadow-sm)] sm:p-6">
      ...
      <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{QUICK_ACTIONS.map((action) => <Link className="group rounded-2xl border border-[var(--ft-border)] bg-[var(--ft-bg-surface)] p-3 transition hover:-translate-y-0.5 hover:border-[var(--ft-accent)]/35 hover:shadow-[var(--shadow-sm)]" href={action.href} key={action.label}><action.icon className="size-4 text-[var(--ft-accent)]" /><div className="mt-3 text-xs font-semibold">{action.label}</div><div className="mt-1 text-[10px] leading-4 text-[var(--ft-text-muted)]">{action.detail}</div><ArrowRight className="mt-3 size-3.5 text-[var(--ft-text-muted)] transition group-hover:translate-x-1 group-hover:text-[var(--ft-accent)]" /></Link>)}</div>
    </section>
