      ...
      <div className="grid min-h-screen lg:grid-cols-[1fr_1fr]">
        <div className="hidden lg:flex lg:flex-col lg:justify-center lg:gap-8 lg:px-16">
          <motion.div animate={{ opacity: 1, y: 0 }} className="max-w-md" initial={{ opacity: 0, y: 20 }} transition={{ duration: 0.5 }}>
            ...
            <h2 className="mt-10 text-4xl font-black tracking-tight">One account for money, services and growth.</h2>
        ...
        <div className="flex flex-col items-center justify-center px-4 py-10 sm:px-8">
          <motion.div animate={{ opacity: 1, y: 0 }} className="w-full max-w-[520px]" initial={{ opacity: 0, y: 12 }} transition={{ duration: 0.35, delay: 0.05 }}>
            ...
            <div className="rounded-[var(--radius-xl)] border border-[var(--ft-border)] bg-[var(--ft-bg-raised)] p-6 shadow-[var(--shadow-lg)] sm:p-8">
              <h1 className="text-2xl font-bold tracking-tight">Create your account</h1>
              <p className="mt-2 text-sm text-[var(--ft-text-secondary)]">Takes less than a minute. We'll take you straight to what brought you here.</p>

              {selectedIntent ? <div className="mt-5 rounded-2xl border border-[var(--ft-accent)]/25 bg-[var(--ft-accent-subtle)] px-4 py-3"><div className="text-[10px] font-bold uppercase tracking-[0.16em] text-[var(--ft-accent)]">Your starting point</div><div className="mt-1 font-semibold">{selectedIntent.label}</div><div className="mt-0.5 text-xs text-[var(--ft-text-secondary)]">{selectedIntent.eyebrow}</div></div> : null}
              ...
              <form className="mt-7 grid gap-5" onSubmit={(event) => void handleSubmit(event)}>
                ...
                <Button className="h-11 w-full justify-center" disabled={submitting || !canSubmit} type="submit">{submitting ? <><motion.div animate={{ rotate: 360 }} className="size-4 rounded-full border-2 border-[var(--ft-text-inverse)]/30 border-t-[var(--ft-text-inverse)]" transition={{ duration: 0.8, repeat: Infinity, ease: "linear" }} />Creating account...</> : <><UserPlus className="size-4" />Create account</>}</Button>
              </form>
              <div className="mt-6 flex items-center justify-center gap-2 text-sm"><span className="text-[var(--ft-text-secondary)]">Already have an account?</span><a className="inline-flex items-center gap-1.5 font-medium text-[var(--ft-accent)]" href="/login">Sign in<ArrowRight className="size-3.5" /></a></div>
            </div>
