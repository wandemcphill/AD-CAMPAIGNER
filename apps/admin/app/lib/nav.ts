 ...
 * union of real routes rather than plain `string`. Any href built at
 * runtime -- from a lookup table, an API response, a template literal over
 * an id -- has type `string` and needs a cast to satisfy that union, even
 * when every value the table can actually produce is a real, valid route
 * (as with trailingSlash: true routes defined in a plain array literal:
 * TypeScript widens `href` to `string` unless the array itself is typed or
 * `as const`).
 *
